...
Silicon via Metal. The 110M model matches Whisper large-v3 accuracy at 14x fewer
...
```
  ...
  parakeet.cpp clone --cmake--> ~/.local/bin/parakeet
                         \----> ~/.local/bin/example-server

  minutes repo ------cargo----> ~/.local/bin/minutes

  minutes setup --parakeet ---> silero_vad_v5.safetensors
  HuggingFace ----download----> <model>.nemo --convert_nemo.py--> <model>/<model>.safetensors
                               <model>.nemo --tar extract------> <model>/<model>.tokenizer.vocab
...
model each time. That is visibly slow.
...
acceleration without waiting on parakeet.cpp CUDA. Contributed by
...
Use the dependency line from Phase 2 inside the active venv:

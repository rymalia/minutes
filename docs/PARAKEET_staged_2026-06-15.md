...
Parakeet locally on Metal," do this. Each step is a separate set of
commands for you to run — do not paste TOML into the shell; step 4 writes a file.
...
- CMake. Any recent CMake works — step 1's `-DAXIOM_INSTALL=OFF
  ...
  instructions nor -latomic found`, see [Troubleshooting](#troubleshooting).
...
#   -DPARAKEET_BUILD_SERVER_EXAMPLE=ON   builds the warm sidecar (off by default)
...
cmake --build build -j
# Copy both binaries (parakeet & example-server).
# Their location differs by generator (Ninja vs Unix Makefiles), so locate
# them rather than hard-coding paths.
# example-server is the warm sidecar: without it, live mode (minutes live and
# the recording sidecar) spawns a fresh subprocess per utterance — visibly slow
# because the model reloads each time.
mkdir -p ~/.local/bin
...
# 3. Install the embedded Silero VAD weights + create the model dir.
minutes setup --parakeet

# 4. Download + convert the tdt-600m model (setup above does not do this).
#    These are the "Install Models" steps below, specialized to tdt-600m.
#    Use a throwaway Python venv so pip and python share one interpreter and
#    the `hf` downloader lands on PATH.
...
cd ~/src/parakeet.cpp                      # the clone from step 1 (still present)
...
python scripts/convert_nemo.py parakeet-tdt-0.6b-v3.nemo \
  -o ~/.minutes/models/parakeet/tdt-600m/tdt-600m.safetensors --model 600m-tdt
...
deactivate
# Reclaim space: the .nemo (~2.4 GB) is not needed once converted.
rm -f parakeet-tdt-0.6b-v3.nemo
# The ~/src/parakeet.cpp clone and ~/parakeet-convert-env are no longer needed by
# the binaries or the runtime — delete them unless you plan to install another
# model or rebuild/update parakeet.cpp.
```
...
# 5. (Optional) Use the known-good CMake 3.31.x. The build also works on CMake
#    4.x with the install-disable flags shown later; 3.31.x is the version that
#    sidesteps the rare Highway atomics probe failure if you happen to hit it.
...
# Put this CMake first on PATH for the parakeet.cpp build:
...
```bash
# Clone with submodules (into ~/src, to match the rest of this guide)
mkdir -p ~/src && cd ~/src
git clone --recursive https://github.com/Frikallo/parakeet.cpp
cd parakeet.cpp

# Configure + build (macOS with Metal). Configure directly with cmake rather
# than `make build` so the two flags below take effect:
#   -DPARAKEET_BUILD_SERVER_EXAMPLE=ON   builds example-server (off by default)
#   -DAXIOM_INSTALL=OFF -DPARAKEET_INSTALL=OFF   skips the axiom install(EXPORT)
#                                                rules that modern CMake rejects
cmake -B build -DCMAKE_BUILD_TYPE=Release \
  -DPARAKEET_BUILD_SERVER_EXAMPLE=ON \
  -DAXIOM_INSTALL=OFF -DPARAKEET_INSTALL=OFF
cmake --build build -j
...
# found", either patch third_party/axiom/third_party/highway/cmake/FindAtomics.cmake
# to short-circuit on Apple arm64, or use CMake 3.31.x — see Troubleshooting below.
...
# Step 1 — install native Silero VAD weights & prepare dir for tdt-600m model  
minutes setup --parakeet
...
nor -latomic found."` We did *not* reproduce this on CMake 4.3.2 or 4.3.3 (the
...
[Prerequisites › macOS](#macos-apple-silicon)).
...
downgrade needed. (Homebrew no longer ships a `cmake@3` formula, so the old
downgrade workaround isn't available anyway.)

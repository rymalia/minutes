...
Optional Apple Silicon sanity check — both binaries should link Metal and
Accelerate:
...
### `hf: command not found` (during model download)
The `hf` downloader lives in the conversion venv. Activate it (and reinstall if
needed), then retry the download:
...
`hf` is only needed during Phase 2 download — never at runtime.
